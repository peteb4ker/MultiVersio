...
Lofi::Lofi(IMultiVersioCommon &mv) : mv(mv)
{
    ...
    lofi_target_Lofi_LFO_Freq = lofi_current_Lofi_LFO_Freq
        = mv.versio.AudioSampleRate();
    ...
    lofi_tone_par.Init(mv.versio.knobs[daisy::DaisyVersio::KNOB_2],
                       20,
                       20000,
                       lofi_tone_par.LOGARITHMIC);
    lofi_rate_par.Init(mv.versio.knobs[daisy::DaisyVersio::KNOB_1],
                       mv.versio.AudioSampleRate() * 4,
                       mv.versio.AudioSampleRate() / 16,
...
void Lofi::processSample(float &outl, float &outr, float inl, float inr)
{
    ...
    this->mv.rev.SetLpFreq((lofi_envelope_follower * 0.6)
                           + (global_sample_rate * 0.3));

    this->mv.svf2l.set_f_q<stmlib::FREQUENCY_FAST>(
        lofi_envelope_follower / global_sample_rate, 1.f);
    this->mv.svf2r.set_f_q<stmlib::FREQUENCY_FAST>(
        lofi_envelope_follower / global_sample_rate, 1.f);
    ...
    daisysp::fonepole(lofi_current_Lofi_LFO_Freq,
    ...
    outl = sqrt(0.5f * (lofi_drywet * 2.0f)) * IMultiVersioCommon::dell.Read()
           + sqrt(0.95f * (2.f - (lofi_drywet * 2))) * inl;
    outr = sqrt(0.5f * (lofi_drywet * 2.0f)) * IMultiVersioCommon::delr.Read()
    ...
    float filter_outl
        = this->mv.svf2l.Process<stmlib::FILTER_MODE_LOW_PASS>(inl);
    float filter_outr
        = this->mv.svf2r.Process<stmlib::FILTER_MODE_LOW_PASS>(inr);
