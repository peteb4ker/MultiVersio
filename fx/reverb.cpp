...
void Reverb::WriteShimmerBuffer2(float in_l, float in_r)
{
    ...
    IMultiVersioCommon::mlooper_frozen_buf_1l[reverb_shimmer_write_pos2]
        = (in_r + in_l) / 2;
...
void Reverb::processSample(float &outl, float &outr, float inl, float inr)
{
    ...
    {
        shimmer_l
            = IMultiVersioCommon::mlooper_buf_1l[reverb_shimmer_play_pos1l];
        shimmer_r
            = IMultiVersioCommon::mlooper_buf_1r[reverb_shimmer_play_pos1r];
    ...
    if(this->mv.mode == REV)
    {
        float octave2_shim = IMultiVersioCommon::mlooper_frozen_buf_1l[(
                                 int)reverb_shimmer_play_pos2]
                             * 0.5f;
        ...
        reverb_shimmer_play_pos2
            = (static_cast<int>(reverb_shimmer_play_pos2) + 4)
              % reverb_shimmer_buffer_size2;
    ...
    {
        float octave2_shim
            = this->mv.mlooper_frozen_buf_1l[(int)reverb_shimmer_play_pos2];
        shimmer_l += octave2_shim;
        shimmer_r += octave2_shim;
    ...
    daisysp::fonepole(
        reverb_target_compression, this->mv.reverb_compression, .001f);
