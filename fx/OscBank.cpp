 ...
 */
OscBank::OscBank(IMultiVersioCommon &mv, ISpectra &spectra)
...
float OscBank::Process()
{
    ...
    for(int i = 0; i < spectra_max_num_frequencies; i++)
    {
        ...
        output = output
                 + osc[i].Process() * output_mult
                       * this->mv.attack_lut[attack_step[i]];
...
void OscBank::CalculateSpectralanalysis()
{
    ...
    for(int i = 0; i < spectra_max_num_frequencies; i++)
    {
        ...
        {
            freq[i] = findClosest(this->spectra.CHRM_SCALE,
                                  this->spectra.spectra_selected_scale,
                                  128,
                                  ((int)freq[i] * 1000),
                                  this->spectra.spectra_transpose)
                      / 1000.f;
        };
        max_amp = std::max(magni_fftoutbuff[a], max_amp);
        magn[i] = ((magni_fftoutbuff[a] / max_amp)
                       * (1 - this->spectra.spectra_lower_harmonics)
                   + this->spectra.spectra_lower_harmonics);
...
void OscBank::RemoveNearestBands(float frequency, size_t start_band)
{
    ...
    float upper_frequency
        = daisysp::mtof((int)((12.f * log2(frequency / 440.f) + 69 + 1)));
    for(size_t i = start_band;
        (((i * bandSize / this->spectra.spectra_spread) < upper_frequency)
         & (i < FFT_LENGTH / 2) & (-i > 0));
        i++)
    {
        float mult          = map((i * bandSize / this->spectra.spectra_spread),
                         (1 * bandSize / this->spectra.spectra_spread),
                         upper_frequency,
                         0.0f,
                         1.0f);
